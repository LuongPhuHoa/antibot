 ...
 * @Last Modified time: 2019-09-15 01:21:20
*/
class Antibot
{
	...
	function login($ApiKey){
		$login 	= $this->httpGet($this->domain.'/api/manage-auth?apikey='.$ApiKey);
		$json 	= $this->json($login);
		if($json['status']){
			$json['apikey'] = $ApiKey;
			return $this->respons(	true , $json );
		}else{
			return $this->respons(	false , $json );
		}
	}
	function shortlink($ApiKey){
		$respons 	= $this->httpGet($this->domain.'/api/manage-shortlink?apikey='.$ApiKey);
		$json 		= $this->json($respons);
		if($json['status']){
			return $this->respons(	true , 	array('message' => 'API KEY VALID' , 'list' => $json['list'] ));
		}else{
			return $this->respons(	false , array('message' => 'API KEY NOT FOUND') );
		}
	}
	function delete($ApiKey , $id){
		$respons 	= $this->httpGet($this->domain.'/api/manage-delete?apikey='.$ApiKey."&id=".$id);
		$json 		= $this->json($respons);
		if($json['status']){
			return $this->respons(	true , 	$json );
		}else{
			return $this->respons(	false , array('message' => $json['message'] ) );
		}
	}
	function view($ApiKey , $id){
		$respons 	= $this->httpGet($this->domain.'/api/manage-view?apikey='.$ApiKey.'&id='.$id);
		$json 		= $this->json($respons);
		if($json['status']){
			return $this->respons(	true , 	$json );
		}else{
	...
	function generate($ApiKey , $post){
		$generate 	= $this->httpPost($this->domain.'/api/manage-generate?apikey='.$ApiKey , $post);
		$json 	= $this->json($generate);
		if($json['status']){
			return $this->respons(	true , $json);
		}else{
			return $this->respons(	false , $json);
	...
	function keyname($ApiKey , $keyname){
		$respons 	= $this->httpGet($this->domain.'/api/manage-key?apikey='.$ApiKey.'&keyname='.$keyname);
		$json 		= $this->json($respons);
		if($json['status']){
			return $this->respons(	true , 	$json );
		}else{
	...
	function check($ApiKey , $keyname , $ip){
		$respons 	= $this->httpGet($this->domain.'/api/manage-visitor?apikey='.$ApiKey.'&keyname='.$keyname.'&ip='.$ip);
		$json 		= $this->json($respons);
		if($json['status']){
			return $this->respons(	true , 	$json );
		}else{
